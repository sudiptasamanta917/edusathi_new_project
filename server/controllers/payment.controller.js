...
export const createPaymentOrder = async (req, res) => {
  try {
    ...
    const order = await rp.orders.create({
      ...
      currency: 'INR',
      receipt: receipt,
      notes: { institute: normInstitute, owner: normOwner, email: normEmail, plan, planId: planId ? String(planId) : null, domain: normDomain, subdomain: subdomain || null, durationDays: durationDays ?? null, billing: billing ?? null, timestamp: new Date().toISOString() },
    });
...
export const verifyPayment = async (req, res) => {
  try {
    ...
    if (razorpay_signature === expectedSignature) {
      ...
      const subdomain = notes.subdomain ? String(notes.subdomain).toLowerCase().trim() : null;
      ...
      if (existingCenter) {
        ...
        if (subdomain) {
          existingCenter.subdomain = subdomain;
        }
        ...
        try {
          ...
          await BusinessPurchase.findOneAndUpdate(
            { razorpay_order_id },
            {
              $set: { status: 'paid', razorpay_payment_id, centerId: existingCenter._id, businessId: (authBizId || (biz ? biz._id : null)), plan, domain, subdomain },
      ...
      try {
        ...
        await BusinessPurchase.findOneAndUpdate(
          { razorpay_order_id },
          {
            $set: { status: 'paid', razorpay_payment_id, centerId: newCenter._id, businessId: (authBizId || (biz ? biz._id : null)), plan, domain, subdomain },
            $setOnInsert: { createdById: actorId, createdByRole: actorRole, createdByEmail, createdByName },
          },
          { upsert: true }
        );
